                ...
                <div className="main-content mt-5">
                    <div >
                        <div className="text-center mx-5 mb-4 mt-5">
                            <h2 className="fw-bold">Requested Delivery</h2>
                        </div>
                        <div className="d-flex justify-content-between align-items-center mt-4">
                            <div className="mt-2 d-flex align-items-center ms-3">
                            ...
                            <div className="me-5">
                                <input
                                    placeholder="Search" className="searchInput me-4" onChange={(e) => setQuery(e.target.value)}
                                />
                            </div>
                        </div>
                            ...
                            <table className="table table-hover table-row-gray-300 align-middle gs-0 gy-4">
                                <thead>
                                    <tr className="fw-bold text-muted TableHead" >
                                        ...
                                        <th className="min-w-100px text-center" style={{ verticalAlign: 'middle' }}>Action </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {TableData.slice((currentPage - 1) * entriesPerPage, currentPage * entriesPerPage).filter((user) =>
                                        Object.values(user).some(
                                            (field) =>
                                                typeof field === 'string' && field.toLowerCase().includes(query.toLowerCase())
                                        )
                                    ).slice(0, entriesPerPage).map((data) => (
                                        <tr key={data.id} style={{ height: "50px" }}>
                                            ...
                                            <td className="text-center">
                                                <button className="btn btn-outline-success mx-2 my-1">Accept</button>
                                                <button className="btn btn-outline-danger mx-2 my-1">Deny</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
