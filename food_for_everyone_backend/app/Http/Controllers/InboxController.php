...
class InboxController extends Controller
{
    ...
    public function deleteInboxes($donation_id, $reciever_id)
    {
        try {

            // $deletedRows = Inbox::where('donation_id', $donation_id) and
            // ->orWhere('reciever_id', $reciever_id)
            // ->orWhere('doner_id',$reciever_id))
            // ->delete();
            $deletedRows = Inbox::where('donation_id', $donation_id)
                ->where(function ($query) use ($reciever_id) {
                    $query->orWhere('reciever_id', $reciever_id)
                        ->orWhere('doner_id', $reciever_id);
                })
                ->delete();
